module RackspaceCloudDns
	class Record
		...
		def self.create(domain_id, name, type, data, priority=nil)
			options = {:records => [:name => name, :type => type, :data => data, :priority => priority]}

			RackspaceCloudDns::Request.request("domains/#{domain_id}/records", :post, options)
		end

		# Edit a record for a domain
		def self.edit(domain_id, record_id, name, data, priority=nil)
			options = {:name => name, :data => data, :priority => priority}
